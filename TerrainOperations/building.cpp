...
namespace terrainops {
	...
	long long build(float* out) {
		...
		float vertical_nw, vertical_ne, vertical_se, vertical_sw;
		...
		for (int x = x1; x < x2; x += density) {
			for (int y = y1; y < y2; y += density) {
				...
				vertical_nw = position.nw.z;
				vertical_ne = position.ne.z;
				vertical_se = position.se.z;
